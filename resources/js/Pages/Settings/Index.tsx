                            ...
                            <ul role="list" className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 ">
                                {group.settings.map((setting) => (
                                    <li key={setting.label} className="col-span-1 flex rounded-md shadow-sm">
                                        ...
                                        <div className="flex flex-1 items-center justify-between truncate rounded-r-md border-b border-r border-t border-gray-200 bg-background">
                                        <div className="flex-1 truncate px-4 py-2 text-sm">
                                            <Link href={setting.href} className="font-medium text-text-900 hover:text-text-600">
                                            {setting.label}
                                            </Link>
                                            <p className="text-text-500">{setting.subtitle}</p>
                                        </div>
                                        <div className="shrink-0 pr-2">
                                            <Link
                                                href={setting.href} 
                                                type="button"
                                                className="inline-flex size-8 items-center justify-center rounded-full bg-transparent bg-background text-text-400 hover:text-text-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                            >
